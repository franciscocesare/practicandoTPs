...
int altaCliente(eCliente vec[], int tam, int idCliente, eLocalidad loc[], int tamLoc)
{
    ...
    else
    {
        ...
        obtenerLocalidad(loc, tamLoc, vec[index].idLocalidad,localidad);
...
int borrarCliente(eCliente vec[], int tam, eLocalidad loc[], int tamLoc)
{
    ...
    else
    {
        ...
        mostrarCliente(vec[index],loc, tamLoc);
...
{
    char descLocalidad [30];
    obtenerLocalidad(loc,tamLoc, x.idLocalidad,descLocalidad);
    printf(" %d  %5s   %10s    %10c    %10s   %10s  %10s \n",
           x.idCliente,
           x.apellido,
           x.nombre,
           x.sexo,
...
void mostrarClientes(eCliente vec[], int tam, eLocalidad loc[], int tamLoc)
{
    ...
    for(int i=0; i < tam; i++)
    {
        if( vec[i].isEmpty == 0)
        {
            mostrarCliente(vec[i], loc, tamLoc);
            flag = 1;
        }
    }
