...
int checkCorreoE (char str[]) {
    int i=0;
    while (str[i]!='\0'){
        if((str[i]<'0' || str[i]>'9')&& (str[i]!=' ') && (str[i]<'a' || str[i]>'z') && str[i]!='@' && str[i]!='.'){ // Chequea por la negativa para return 0
                return 0;
        }
        i++;
    }
...
int aux;
aux=getChar("\nDesea continuar? para finalizar ingrese 'n': ");
aux=toupper(aux);
return aux;
}
